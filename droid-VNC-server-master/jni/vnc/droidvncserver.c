...
void initGrabberMethod()
{
  if (method == AUTO) {
    ...
    if (initFlinger() != -1){
    	method = FLINGER;
    	L("**********************");//tina add
    	L("tina: method = FLINGER \n");//tina add
    }
    else if (initGralloc()!=-1){
        method = GRALLOC;
    	L("**********************");//tina add
    	L("tina: method = GRALLOC \n");//tina add
    }
    else if (initFB() != -1) {
    	method = FRAMEBUFFER;
    	L("**********************");//tina add
        L("tina: method = FRAMEBUFFER \n");//tina add
    }
    #if 0
    else if (initADB() != -1) {
      method = ADB;
...
int main(int argc, char **argv)
{
  ...
  if(argc > 1) {
    ...
    while (1) {
        ...
        //tina add. rfbProcessEvents() defines in "jni/vnc/LibVNCServer-0.9.9/libvncserver/main.c"
